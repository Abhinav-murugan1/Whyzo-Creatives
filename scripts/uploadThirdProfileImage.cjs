...
const sourceImage = path.resolve(__dirname, '../public/team/anzil-s.jpg');
...
const localCopy = path.join(destDir, 'member_03.jpg');
fs.copyFileSync(sourceImage, localCopy);
console.log('Saved local copy to:', localCopy);

async function upload() {
  try {
    console.log('Uploading third profile image to Cloudinary...');
