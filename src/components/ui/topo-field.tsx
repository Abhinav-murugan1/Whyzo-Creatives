...
/**
 * The source document is a full marketing page that loads Tailwind, Iconify, GSAP and ScrollTrigger
 * from three CDNs. The isolate step below throws every one of those nodes away and keeps only
 * #topo-canvas, so the network cost buys nothing. Drop the tags and stub the two globals the inline
 * script still touches — without the stub, `gsap.registerPlugin` throws and execution never reaches
 * the WebGL block that actually draws the field.
 */
function stripUnusedDependencies(source: string) {
  return source
    .replace(/[ \t]*<script src="https:\/\/[^"]*"><\/script>\n?/g, "")
    .replace(
      "gsap.registerPlugin(ScrollTrigger);",
      "if (typeof gsap === 'undefined') { window.gsap = { registerPlugin: function () {}, to: function () {} }; window.ScrollTrigger = null; }\n        gsap.registerPlugin(ScrollTrigger);",
    );
}
...
function buildFocusedDocument(knobs: {
  ...
  const patched = patchTopoField(stripUnusedDependencies(topoFieldSource), knobs);
