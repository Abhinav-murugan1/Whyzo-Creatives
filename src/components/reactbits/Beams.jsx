...
const degToRad = THREE.MathUtils?.degToRad || (deg => (deg * Math.PI) / 180);
...
const MergedPlanes = forwardRef(({ material, width, count, height }, ref) => {
  ...
  const geometry = useMemo(
    () => createStackedPlanesBufferGeometry(count, width, height, 0, 45),
    [count, width, height]
  );
  useFrame((_, delta) => {
    ...
    if (mesh.current?.material?.uniforms?.time) {
      ...
      mesh.current.material.uniforms.time.value += 0.1 * safeDelta;
    }
  });
